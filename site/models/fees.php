...
if (empty($list['direction']))
{
    ...
    $query->join('LEFT', '#__users AS uc ON uc.id=a.checked_out');
		...
		$query->select('#__fee_semester_1887491.title AS semesters_title_1887491');
		...
		$query->select('#__fee_year_1887494.start AS years_title_1887494');
...
if (!JFactory::getUser()->authorise('core.edit.state', 'com_fee'))
{
	...
	public function getItems()
	{
		...
		foreach($items as $item){
			...
			$item->student_alias = !empty($textValueId) ? implode(', ', $textValueId) : $item->student_alias;
			$item->title = !empty($textValue) ? implode(', ', $textValue) : $item->title;
