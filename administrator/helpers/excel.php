...
class FeeHelperExcel {
    ...
    public static function readExcel($path) {
        ...
        for ($row = 4; $row <= $highestRow; $row++) {
            ...
            /* @var $sheet type */
            if ($sheet->getCellByColumnAndRow(4, $row - 1)->getValue() && $sheet->getCellByColumnAndRow(8, $row - 1)->getValue()) {
                if ($department != $sheet->getCellByColumnAndRow(4, $row - 1)->getValue() && $course != $sheet->getCellByColumnAndRow(8, $row - 1)->getValue()) {
                    $i+=2;
                    $j = 0;
                }
            }
