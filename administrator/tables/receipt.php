 ...
 * receipt Table class
 */
class FeeTablereceipt extends JTable {
    ...
    public function bind($array, $ignore = '') {
        ...
        if (($task == 'save' || $task == 'apply') && (!JFactory::getUser()->authorise('core.edit.state', 'com_fee.receipt.' . $array['id']) && $array['state'] == 1)) {
            $array['state'] = 0;
        }
        if ($array['id'] == 0) {
        ...
        if (!JFactory::getUser()->authorise('core.admin', 'com_fee.receipt.' . $array['id'])) {
            $actions = JFactory::getACL()->getActions('com_fee', 'receipt');
            $default_actions = JFactory::getACL()->getAssetRules('com_fee.receipt.' . $array['id'])->getData();
            $array_jaccess = array();
    ...
    protected function _getAssetName() {
        ...
        return 'com_fee.receipt.' . (int) $this->$k;
    }
    ...
    public function store($updateNulls = false) {
        ...
        if ($result) {
            return $this->updateOwed();
        }
        return $result;
    }

    public function updateOwed() {
        $db = JFactory::getDbo();
        ...
        if ($result) {
            ...
            $query
                    ->update('`#__fee_fee`')
                    ->set('`owed` = `payable`-(`payable`*`rate`/100) -' . $db->quote($db->escape($result)))
                    ->where('`student_alias` = ' . $db->quote($db->escape($this->student_alias)))
                    ->where('`semester_alias` = ' . $db->quote($db->escape($this->semester_alias)))
                    ->where('`year_alias` = ' . $db->quote($db->escape($this->year_alias)));
